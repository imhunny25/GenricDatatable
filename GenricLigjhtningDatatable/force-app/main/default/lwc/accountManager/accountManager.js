...
export default class AccountManager extends LightningElement {
    ...
    wiredPicklists({ data, error }) {
        if (data) {
            ...
            for (let field in data) {
                this.picklistFieldValues[field] = data[field].map(v => ({ label: v, value: v }));
            }
    ...
    wiredAccessibleFields({ data, error }) {
        if (data) {
            ...
            if (!this.userCustomizedFields) {
                this.initiallyFields_for_datatable = this.pickDefaultFields(this.accessibleFields, 4);
                this.values = [...this.initiallyFields_for_datatable];
            }
            console.log('Initial Fields:', JSON.stringify(this.initiallyFields_for_datatable));
            ...
            let fieldOptions = this.initiallyFields_for_datatable.map(fieldApiName => {
                // Find the field in array A by apiName
                let fieldDef = this.accessibleFields.find(f => f.apiName === fieldApiName);
                if (fieldDef) {
                    ...
                    if (Array.isArray(this.accessibleFields)) {
                        this.accessibleFields.forEach(field => {
                            if (field && (field.type === 'PICKLIST' || field.type === 'Multipicklist')) {
                                let optionKey = field.apiName.charAt(0).toLowerCase() + field.apiName.slice(1) + 'Options';
                                //const optionsKey = `${field.apiName}Options`;
                                enriched[optionKey] = this.picklistFieldValues[field.apiName] || [];
                            }
                        });
                    }
                    return enriched;
    ...
    handleSort(event) {
        ...
        handleFieldChange(event) {
            ...
            let fieldDef = this.accessibleFields.find(f => f.apiName === this.selectedField);
    ...
    handleSave(event) {
        ...
        updateAccounts({ sObjList: updatedFields })
            ...
            .catch(error => {
                this.showToast('Error updating records', error.body?.message || error.message, 'error');
            });
    }

    // ⏮️ Pagination
    prevPage() { if (this.pageNumber > 1) { this.pageNumber--; this.loadAccounts(); } }
    nextPage() { if (this.pageNumber < this.totalPages) { this.pageNumber++; this.loadAccounts(); } }
    ...
    buildColumns(fieldMetadata) {
        ...
        fieldMetadata.forEach(field => {
            if (this.initiallyFields_for_datatable.includes(field.apiName)) {
                let col = {
                    label: field.label,
                ...
                if (field && (field.type === 'DATE' || field.type === 'Date')) {
                    col.type = 'date-local';
                    col.typeAttributes = {
                        month: '2-digit',
                        day: '2-digit'
                    };
                } else if (field && (field.type === 'DATETIME' || field.type === 'Datetime' || field.type === 'DateTime')) {
                    col.type = 'datetime';
                    // Note: lightning-datatable uses type 'date' for datetime as well; time parts render when provided via typeAttributes [web:9][web:1].
                    col.typeAttributes = {
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit',
                ...
                // Generic picklist handling (Apex returns 'Picklist'/'Multipicklist')
                if (field && (field.type === 'PICKLIST' || field.type === 'Multipicklist')) {
                    let optionKey = field.apiName.charAt(0).toLowerCase() + field.apiName.slice(1) + 'Options';
                    ...
                    col.typeAttributes = {
                        ...
                        options: { fieldName: optionKey }  // ✅ will give industryOptions instead of IndustryOptions
                    };
                }
                cols.push(col);
            }
    ...
    scoreField(f) {
        ...
        if (['STRING','PICKLIST','PHONE','EMAIL','URL'].includes(f.type)) s += 12;
    ...
    pickDefaultFields(accessibleFields, count = 4) {
        ...
        return picked.length ? picked : (accessibleFields || []).slice(0, count).map(f => f.apiName);
    }
